// ════════════════════════════════════════════════════════════
//  BillsScreen — تتبع الفواتير والاشتراكات
// ════════════════════════════════════════════════════════════
...
class BillsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('الفواتير والاشتراكات'),
              ...
              children: [
                ...
                WaCard(child: Row(children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('مستحق هذا الشهر',
                      style: TextStyle(fontSize: 12, color: WaColors.textMuted)),
                    const SizedBox(height: 4),
                    Text(ap.fmt(monthTotal),
                      style: const TextStyle(
                        fontSize: 22, fontWeight: FontWeight.w800, color: WaColors.gold)),
                  ])),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    _SummaryChip('${overdue.length + today.length} متأخرة/اليوم', WaColors.danger),
                    const SizedBox(height: 4),
                    _SummaryChip('${upcoming.length} قادمة',  WaColors.info),
                  ]),
                ...
                if (overdue.isNotEmpty) ...[
                  _GroupHeader('🔴 متأخرة', WaColors.danger),
                ...
                // ── اليوم ───────────────────────────
                if (today.isNotEmpty) ...[
                  _GroupHeader('🟡 مستحقة اليوم', WaColors.warning),
                ...
                // ── خلال 3 أيام ─────────────────────
                if (soon.isNotEmpty) ...[
                  _GroupHeader('🔔 خلال 3 أيام', WaColors.info),
                ...
                if (upcoming.isNotEmpty) ...[
                  _GroupHeader('📅 قادمة', WaColors.textMuted),
...
class _BillCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final daysLabel = bill.isOverdue
        ? 'متأخرة ${bill.daysUntilDue.abs()} يوم'
        : bill.isDueToday
            ? 'اليوم'
            : 'بعد ${bill.daysUntilDue} يوم';
    ...
    return Dismissible(
      ...
      confirmDismiss: (_) => showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text('هل تريد حذف "${bill.name}"؟'),
          actions: [
        ...
        child: WaCard(child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text(ap.fmt(bill.amount),
              style: const TextStyle(
                  fontSize: 14, fontWeight: FontWeight.w800, color: WaColors.danger)),
            const SizedBox(height: 4),
            GestureDetector(
              onTap: () => _confirmPay(context, ap, bill),
              child: Container(
  ...
  void _confirmPay(BuildContext ctx, AppProvider ap, BillModel b) =>
    showDialog(
      context: ctx,
      builder: (_) => AlertDialog(
        title  : const Text('تأكيد الدفع'),
        content: Text(
          'دفع "${b.name}" بمبلغ ${ap.fmt(b.amount)} ${ap.currency}؟\n'
          'سيُسجَّل كمصروف تلقائياً.',
        ),
        actions: [
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 12),
      const Text('لا توجد فواتير مضافة',
        style: TextStyle(fontSize: 15, color: WaColors.textMuted)),
      const SizedBox(height: 6),
      const Text('أضف فواتيرك واشتراكاتك وسنذكّرك قبل الاستحقاق',
        style: TextStyle(fontSize: 12, color: WaColors.textMuted),
...
class _AddBillSheetState extends State<_AddBillSheet> {
  ...
  void initState() {
    ...
    _billCats = kExpenseCategories.where((c) => [
      'bills', 'transport', 'health', 'education',
      'entertainment', 'food', 'shopping', 'other',
    ].contains(c.id)).toList();
    // تأكد أن القيمة الافتراضية موجودة في القائمة
    _cat = _billCats.any((c) => c.id == 'bills')
        ? 'bills'
        : _billCats.first.id;
  }
  ...
  Future<void> _save() async {
    ...
    if (_nameCtrl.text.trim().isEmpty) { _snack('أدخل اسم الفاتورة'); return; }
    ...
    try {
      final b = BillModel(
        ...
        freq: _freq, dueDate: _date, isPaid: false,
        ...
        child: Column(mainAxisSize: MainAxisSize.min,
          ...
          children: [
            Center(child: Container(width: 36, height: 4,
              decoration: BoxDecoration(color: WaColors.border,
                  borderRadius: BorderRadius.circular(2)))),
            const SizedBox(height: 16),
            const Text('فاتورة / اشتراك',
            ...
            TextField(
              ...
              decoration : InputDecoration(
                labelText: 'الاسم',
                hintText : 'مثال: نتفليكس، إيجار، كهرباء...',
            ...
            Row(children: [
              ...
              Expanded(child: DropdownButtonFormField<BillFreq>(
                ...
                items: const [
                  DropdownMenuItem(value: BillFreq.once,    child: Text('مرة واحدة')),
                  DropdownMenuItem(value: BillFreq.weekly,  child: Text('أسبوعي')),
                  DropdownMenuItem(value: BillFreq.monthly, child: Text('شهري')),
                  DropdownMenuItem(value: BillFreq.yearly,  child: Text('سنوي')),
                ],
              ...
              child: Container(
                ...
                child: Row(children: [
                  const Icon(Icons.calendar_today_outlined,
                    size: 18, color: WaColors.goldDim),
                  const SizedBox(width: 8),
                  Text('تاريخ الاستحقاق: $_date',
